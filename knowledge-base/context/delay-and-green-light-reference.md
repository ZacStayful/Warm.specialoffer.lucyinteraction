...
**DLY-MOV** — Moving abroad / leaving residence (n=8, eventual conversion 13% — lowest of the 5)
- Top triggers: relocating with a firm departure date already set (2); needs a remote/hands-off solution while away (2)
...
**DLY-OTH** — Other prerequisite / catch-all (n=24, eventual conversion 38% — highest of the 5)
- Top triggers: renovation/building work incomplete (8); mortgage/financing unresolved (6); third-party consent needed — freeholder/agent (5); partner sign-off needed (4)
...
- Reframe: name the specific blocker back precisely (builder, lender, freeholder, spouse) rather than treating it as generic hesitation
...
**DLY-TEN** — Existing tenant (n=7, eventual conversion 14%)
- Top triggers: tenant/occupant not due to move out for months (3); legally unable to serve notice, only natural turnover or sale (1); tenant verbally agreed but no formal notice served (1)
...
**GL-MGMT** — wants fully hands-off, full management. Near-universal once present: 13/14 analytical-evaluators, 10/10 certainty-seekers. Why it's a green light: the lead has already accepted the core value proposition (someone else runs it) rather than negotiating service scope.

**GL-TIME** — an external forcing deadline (mortgage renewal, relocation, property already empty or on the market). The most frequent green light aggregated across by-profile-type files (~29 vs GL-MGMT's ~28). Why: gives the lead their own reason to decide now, rather than Stayful manufacturing urgency.

**GL-SELF** — the lead proactively signals readiness (asks about a second-property discount, raises signing before the next call, volunteers next steps unprompted). Rarer (~12 aggregate) but the highest-intent signal precisely because it's unprompted.

## Combination Patterns (offer-timing-intelligence.json, n=6 offers with recorded green-light data)
...
- Small n (6) — do not read this as proof combinations don't drive acceptance; it likely reflects the general deferred-decision pattern across the dataset (overall offer acceptance is 2.3%, see offer-triggers-reference.md).
